<?php
require_once __DIR__ . '/../../controlador/alumno/AlumnoController.php';
?>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['crear'])) {
        $controller->crear($_POST['nombre'], $_POST['apellido'], $_POST['email']);
        header("Location: " . $_SERVER['PHP_SELF']);
        exit();
    } elseif (isset($_POST['actualizar'])) {
        $controller->actualizar($_POST['id'], $_POST['nombre'], $_POST['apellido'], $_POST['email']);
        header("Location: " . $_SERVER['PHP_SELF']);
        exit();
    ...
    <div class="row">
        <div class="col col-12">
            <div class="card shadow-sm">
                <div class="card-header bg-primary text-white">
                    <h3 class="mb-0">Lista de Alumnos</h3>
                </div>
                            ...
                            <tr>
                                ...
                                <td class="actions">
                                    <form method="post" action="" class="d-inline">
                                        ...
                                        <input type="text" name="apellido" value="<?= htmlspecialchars($row['apellido']) ?>" required class="form-control form-control-sm d-inline-block" style="width: 150px;">
                                        <input type="email" name="email" value="<?= htmlspecialchars($row['email']) ?>" required class="form-control form-control-sm d-inline-block" style="width: 200px;">
                                    ...
                                    <form method="post" action="" class="d-inline ms-2">
                                        <input type="hidden" name="id" value="<?= $row['id'] ?>">
                                        <button type="submit" name="eliminar" class="btn btn-danger btn-sm" onclick="return confirm('¿Está seguro de eliminar este alumno?');">
        ...
        <h2 class="h4 mb-3">Lista de Alumnos</h2>
                ...
                <tr>
                    ...
                    <td class="actions">
                        <form method="post" action="" class="d-inline">
                            ...
                            <input type="text" name="apellido" value="<?= htmlspecialchars($row['apellido']) ?>" required class="form-control form-control-sm d-inline-block" style="width: 150px;">
                            <input type="email" name="email" value="<?= htmlspecialchars($row['email']) ?>" required class="form-control form-control-sm d-inline-block" style="width: 200px;">
                            <button type="submit" name="actualizar" class="btn btn-primary btn-sm ms-2">Actualizar</button>
                        </form>
                        <form method="post" action="" class="d-inline ms-2">
                            <input type="hidden" name="id" value="<?= $row['id'] ?>">
                            <button type="submit" name="eliminar" class="btn btn-danger btn-sm" onclick="return confirm('¿Está seguro de eliminar este alumno?');">Eliminar</button>
                        </form>
                    </td>
                </tr>
            <?php endwhile; ?>
